    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (trialBtnList.length) {
                trialBtnList.forEach((trialBtn) => {
                    trialBtn.addEventListener('click', (e) => {
                        ...
                        fetch(url).then((response) => {
                            ...
                            if (hasVideo) {
                                let streamUrl = "{{ route('courses.stream.trial', ':id') }}";
                                streamUrl = streamUrl.replace(':id', data.id);

                                modalEl.querySelector('.modal-body').innerHTML = `
                                        <video id="trial-player" class="video-js vjs-default-skin vjs-big-play-centered" controls autoplay>
                                            <source src="${streamUrl}" type="video/mp4">
                                        </video>
                                    `;
                                videojs('trial-player', { fluid: true, responsive: true });
                            } else {
                ...
                // Reset modal khi đóng — dispose Video.js trước khi xóa DOM
                modalEl.addEventListener('hidden.bs.modal', event => {
                    const existingPlayer = videojs.getPlayer('trial-player');
